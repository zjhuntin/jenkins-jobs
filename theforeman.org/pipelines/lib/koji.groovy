def status_koji_links(repo) {
    ...
    for (String task: tasks) {
        taskinfo = sh(returnStdout: true, script: "koji taskinfo -v ${task}", label: "koji taskinfo").trim()
...
def get_koji_tasks() {
    ...
    if(fileExists('kojilogs')) {
        tasks = sh(returnStdout: true, script: "ls kojilogs -1 |grep -o '[0-9]*\$'", label: 'get koji tasks').trim().split()
    }
    return tasks
}
